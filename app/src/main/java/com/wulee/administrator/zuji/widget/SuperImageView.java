 ...
 *  2、手势缩放
 */
public class SuperImageView extends AppCompatImageView implements RequestListener<String, GlideDrawable> {
    ...
    private void init(AttributeSet attrs) {
        ...
        final TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.SuperImageView);
        if (typedArray.hasValue(R.styleable.SuperImageView_show_progress)) {
            showProgressBar = typedArray.getBoolean(R.styleable.SuperImageView_show_progress, true);
            progressBar = new ProgressBar(getContext(), null, android.R.attr.progressBarStyleSmall);
//            progressBar.setVisibility(GONE);
        }

        if (typedArray.hasValue(R.styleable.SuperImageView_error_res)) {
            errorRes = typedArray.getResourceId(R.styleable.SuperImageView_error_res, 0);
        }
        if (typedArray.hasValue(R.styleable.SuperImageView_placeholder_res)) {
            placeHolderRes = typedArray.getResourceId(R.styleable.SuperImageView_placeholder_res, 0);
        }
    }
    ...
    public boolean canScrollHorizontally(int direction) {
        if (mPinchMode == SuperImageView.PINCH_MODE_SCALE) {
            return true;
        }
    ...
    public interface OuterMatrixChangedListener {
         ...
         */
        void onOuterMatrixChanged(SuperImageView pinchImageView);
    }
     ...
     * 在onTouchEvent末尾被执行.
     */
    private GestureDetector mGestureDetector = new GestureDetector(SuperImageView.this.getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public void onLongPress(MotionEvent e) {
            ...
            if (mOnLongClickListener != null) {
                mOnLongClickListener.onLongClick(SuperImageView.this);
            }
        }
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            if (mOnClickListener != null) {
                mOnClickListener.onClick(SuperImageView.this);
            }
            return true;
        }
    });
